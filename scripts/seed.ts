...
import { hashPassword, hashConsentCode } from '../lib/auth';
...
async function seed() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    const taskType = taskTypes[Math.floor(Math.random() * taskTypes.length)] as 'HOME_VISIT' | 'IMMUNIZATION' | 'FOLLOW_UP';
